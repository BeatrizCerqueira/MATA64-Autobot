...
public class Edit_Interactive extends AdvancedRobot {
	...
	public void turnRange() {
		...
		double xBase = Math.toDegrees(Math.asin(-1 * Math.signum(x)));
		double yBase = Math.toDegrees(Math.acos(-1 * Math.signum(y)));		
		
		out.println(xBase + " " + yBase);
		

		
		double xDegrees = Math.toDegrees(Math.atan(x / y));	
		double yDegrees = Math.toDegrees(Math.atan(y / x));
